...
function buildPrintHtml(title, headers, rows) {
  ...
  // District view: show region-specific stats; Region view: show global stats
  const activeDistricts = isDistrict
    ? r.districts.filter(d => d.workers_count > 0 || d.vacancies_count > 0).length
    : 0;

  const summaryHtml = isDistrict ? `
    <div class="summary-grid">
      <div class="summary-card">
        ...
        <div>
          <div class="summary-label">${t('workers.workersCount')}</div>
          <div class="summary-value" style="color:#0d9488;">${(r.workers_count || 0).toLocaleString()}</div>
        </div>
      </div>
      <div class="summary-card">
        ...
        <div>
          <div class="summary-label">${t('workers.vacanciesCount')}</div>
          <div class="summary-value" style="color:#2563eb;">${(r.vacancies_count || 0).toLocaleString()}</div>
        </div>
      </div>
      <div class="summary-card">
        <div class="summary-icon" style="background:#faf5ff;color:#7c3aed;">
          <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5M3.75 3v18m4.5-18v18m4.5-18v18m4.5-18v18M6 6.75h.008v.008H6V6.75zm0 3h.008v.008H6V9.75zm0 3h.008v.008H6v-.008zm0 3h.008v.008H6v-.008zm4.5-9h.008v.008H10.5V6.75zm0 3h.008v.008H10.5V9.75zm0 3h.008v.008H10.5v-.008zm0 3h.008v.008H10.5v-.008zm4.5-9h.008v.008H15V6.75zm0 3h.008v.008H15V9.75zm0 3h.008v.008H15v-.008zm0 3h.008v.008H15v-.008z"/></svg>
        </div>
        <div>
          <div class="summary-label">${t('workers.districtsCount')}</div>
          <div class="summary-value" style="color:#7c3aed;">${r.districts.length}</div>
        </div>
      </div>
      <div class="summary-card">
        <div class="summary-icon" style="background:#f0fdf4;color:#16a34a;">
          <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </div>
        <div>
          <div class="summary-label">FAOL TUMANLAR</div>
          <div class="summary-value" style="color:#16a34a;">${activeDistricts}</div>
        </div>
      </div>
    </div>` : `
    <div class="summary-grid">
...
<body>
  ...
  <div class="footer">
    ...
    <div>${isDistrict ? `${r.name_uz} — ${r.districts.length} ta tuman/shahar` : `${t('workers.regions')}: ${s.total_regions || regions.value.length}`} | ${date}</div>
  </div>
</body>
</html>`;
}

...
import { ArrowLeftIcon, LanguageIcon, SparklesIcon, ChevronDownIcon, ChevronRightIcon, MagnifyingGlassIcon, MapPinIcon } from '@heroicons/vue/24/outline';
...
async function initMap(lat, lng) {
  destroyMap();
  await loadLeaflet();
  // Wait for DOM to fully render the map container
  await nextTick();
...
async function loadVacancy() {
  ...
  try {
    ...
    // Set category from vacancy data
    const parentCat = rawCategories.value.find(c =>
      c.children?.some(ch => ch.slug === v.category)
    );
    if (parentCat) {
      form.value.parent_category = parentCat.slug;
      const child = parentCat.children.find(ch => ch.slug === v.category);
      if (child) {
        form.value.category_id = { slug: child.slug, name: child.name_uz };
      }
    }
    ...
    // Init map with existing coordinates or district coordinates
    const mapLat = v.latitude ? parseFloat(v.latitude) : form.value.district_id?.latitude;
    const mapLng = v.longitude ? parseFloat(v.longitude) : form.value.district_id?.longitude;
    if (mapLat && mapLng) {
      form.value.latitude = v.latitude ? parseFloat(v.latitude) : null;
      form.value.longitude = v.longitude ? parseFloat(v.longitude) : null;
      nextTick(() => initMap(mapLat, mapLng));
    }

    // Boolean & date fields
    form.value.is_featured = !!v.is_top;
    form.value.require_questionnaire = !!v.has_questionnaire;

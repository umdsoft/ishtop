    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-[1.1] tracking-tight">
          <span class="text-yellow-300 drop-shadow-[0_0_25px_rgba(253,224,71,0.5)]">{{ $t('hero_title_accent') }}</span><br class="sm:hidden">
          {{ $t('hero_title_rest') }}
        </h1>
      ...
      <!-- Stats -->
      <div v-if="stats" class="flex items-center justify-center gap-6 sm:gap-10">
        <div class="text-center">
          <template v-if="loading">
            <div class="h-9 w-16 bg-white/20 rounded animate-pulse mx-auto"></div>
          </template>
          <template v-else>
            <div class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight">{{ formatNum(stats.vacancies) }}+</div>
          </template>
          <div class="text-brand-200 text-xs sm:text-sm mt-0.5">{{ $t('total_vacancies') }}</div>
        </div>
        ...
        <div class="text-center">
          ...
          <template v-else>
            <div class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight">{{ formatNum(stats.companies) }}+</div>
          </template>
          <div class="text-brand-200 text-xs sm:text-sm mt-0.5">{{ $t('total_companies') }}</div>
        </div>
        <div class="w-px h-10 sm:h-12 bg-white/20"></div>
        <div class="text-center">
          <template v-if="loading">
          ...
          <template v-else>
            <div class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight">{{ formatNum(stats.workers) }}+</div>
          </template>
          <div class="text-brand-200 text-xs sm:text-sm mt-0.5">{{ $t('total_workers') }}</div>
        </div>
      </div>
    </div>
  </section>

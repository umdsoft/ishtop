      ...
      <div class="flex items-center justify-between h-16">
        ...
        <div class="hidden md:flex items-center gap-6">
          ...
          <button
            ...
            class="inline-flex items-center gap-2 border border-brand-500 text-brand-500 hover:bg-brand-50 px-4 py-2 rounded-xl font-medium transition-colors text-sm"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <div v-if="mobileOpen" class="md:hidden border-t border-surface-100 bg-white">
        <div class="px-4 py-4 space-y-3">
          ...
          <button
            ...
            class="flex items-center justify-center gap-2 w-full border border-brand-500 text-brand-500 hover:bg-brand-50 px-4 py-3 rounded-xl font-medium transition-colors text-sm"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 9V5.25A2.25 2.25 0 0 1 10.5 3h6a2.25 2.25 0 0 1 2.25 2.25v13.5A2.25 2.25 0 0 1 16.5 21h-6a2.25 2.25 0 0 1-2.25-2.25V15M12 9l3 3m0 0-3 3m3-3H2.25"/>
            </svg>
    ...
    >
      <div v-if="showLoginModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 sm:p-8">
          <!-- Close -->
          <button @click="showLoginModal = false" class="absolute top-4 right-4 text-surface-400 hover:text-surface-600">
          ...
          <div class="text-center mb-6">
            <h2 class="text-xl font-bold text-surface-900">{{ $t('login_modal_title') }}</h2>
            <p class="text-sm text-surface-500 mt-1">{{ $t('login_modal_desc') }}</p>
          </div>

          <div class="space-y-3">
            <!-- Job seeker → Telegram bot -->
            <a
              href="https://t.me/kadrgobot"
              target="_blank"
              @click="showLoginModal = false"
              class="flex items-center gap-4 w-full p-4 rounded-xl border-2 border-surface-200 hover:border-brand-500 hover:bg-brand-50/50 transition-all group"
            >
              ...
              <div class="text-left">
                <div class="font-semibold text-surface-900 group-hover:text-brand-600">{{ $t('login_job_seeker') }}</div>
                <div class="text-sm text-surface-500">{{ $t('login_job_seeker_desc') }}</div>
              </div>
              <svg class="w-5 h-5 text-surface-300 group-hover:text-brand-500 ml-auto flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
              </svg>
            </a>

            <!-- Recruiter → Panel login -->
            <a
            ...
            >
              ...
              <div class="text-left">
                <div class="font-semibold text-surface-900 group-hover:text-brand-600">{{ $t('login_recruiter') }}</div>
                <div class="text-sm text-surface-500">{{ $t('login_recruiter_desc') }}</div>
              </div>
              <svg class="w-5 h-5 text-surface-300 group-hover:text-brand-500 ml-auto flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
              </svg>
